...
DO $$ 
BEGIN 
    -- Add is_active to vendors if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'vendors' AND column_name = 'is_active') THEN
        ALTER TABLE public.vendors ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
    
    -- Add rating to vendors if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'vendors' AND column_name = 'rating') THEN
        ALTER TABLE public.vendors ADD COLUMN rating NUMERIC(3,2) DEFAULT 0;
    END IF;
    
    -- Add updated_at to vendors if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'vendors' AND column_name = 'updated_at') THEN
        ALTER TABLE public.vendors ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT now();
    END IF;
END $$;
