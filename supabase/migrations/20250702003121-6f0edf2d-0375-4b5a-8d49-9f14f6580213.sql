-- Phase 1: Enhanced Database Performance Optimization
-- Add indexes for better query performance

-- Product search and filtering indexes
CREATE INDEX CONCURRENTLY idx_products_name_search ON public.products USING GIN(to_tsvector('english', name));
CREATE INDEX CONCURRENTLY idx_products_category_status ON public.products(category_id, status) WHERE status = 'active';
CREATE INDEX CONCURRENTLY idx_products_vendor_created ON public.products(vendor_id, created_at DESC);
CREATE INDEX CONCURRENTLY idx_products_price_range ON public.products(price) WHERE status = 'active';

-- Order management indexes
CREATE INDEX CONCURRENTLY idx_orders_user_status ON public.orders(user_id, status, created_at DESC);
CREATE INDEX CONCURRENTLY idx_orders_vendor_date ON public.orders(vendor_id, created_at DESC);
CREATE INDEX CONCURRENTLY idx_order_items_product ON public.order_items(product_id, order_id);

-- Analytics and reporting indexes
CREATE INDEX CONCURRENTLY idx_analytics_events_user_date ON public.analytics_events(user_id, created_at DESC);
CREATE INDEX CONCURRENTLY idx_analytics_events_category ON public.analytics_events(event_category, event_name, created_at DESC);

-- User and session indexes
CREATE INDEX CONCURRENTLY idx_user_sessions_expires ON public.user_sessions(expires_at) WHERE expires_at > now();
CREATE INDEX CONCURRENTLY idx_profiles_role_status ON public.profiles(role, created_at DESC);

-- Commission and financial indexes
CREATE INDEX CONCURRENTLY idx_vendor_commissions_date ON public.vendor_commissions(vendor_id, transaction_date DESC);
CREATE INDEX CONCURRENTLY idx_commission_analytics_period ON public.commission_analytics(vendor_id, analytics_period, analytics_date DESC);

-- Search optimization indexes
CREATE INDEX CONCURRENTLY idx_search_queries_user ON public.search_queries(user_id, created_at DESC);
CREATE INDEX CONCURRENTLY idx_search_queries_term ON public.search_queries USING GIN(to_tsvector('english', query_text));

-- Cart and wishlist indexes
CREATE INDEX CONCURRENTLY idx_cart_items_user ON public.cart_items(user_id, created_at DESC);
CREATE INDEX CONCURRENTLY idx_wishlist_user_product ON public.wishlist_items(user_id, product_id);

-- Fraud detection indexes
CREATE INDEX CONCURRENTLY idx_fraud_alerts_status ON public.fraud_alerts(status, created_at DESC);
CREATE INDEX CONCURRENTLY idx_fraud_detection_logs_risk ON public.fraud_detection_logs(risk_level, created_at DESC);
...
CREATE INDEX idx_enhanced_rate_limits_lookup ON public.enhanced_rate_limits(key_type, key_value, endpoint, window_start);
CREATE INDEX idx_enhanced_rate_limits_cleanup ON public.enhanced_rate_limits(window_start);
...
CREATE INDEX idx_query_performance_hash ON public.query_performance_logs(query_hash, created_at DESC);
CREATE INDEX idx_query_performance_time ON public.query_performance_logs(execution_time_ms DESC, created_at DESC);
