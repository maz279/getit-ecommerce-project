-- Create missing tables for Next Phase Implementation

-- Advanced Fraud Detection Events (only if not exists)
CREATE TABLE IF NOT EXISTS public.advanced_fraud_detection (
...
CREATE TABLE IF NOT EXISTS public.vendor_marketplace_tools (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.voice_search_analytics (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID,
  session_id TEXT,
...
CREATE TABLE IF NOT EXISTS public.ar_vr_interactions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID,
  session_id TEXT,
...
CREATE TABLE IF NOT EXISTS public.mobile_app_analytics (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID,
  device_id TEXT,
  app_version TEXT,
...
CREATE TABLE IF NOT EXISTS public.ml_model_performance_tracking (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_advanced_fraud_detection_score ON public.advanced_fraud_detection(fraud_score DESC);
CREATE INDEX IF NOT EXISTS idx_vendor_marketplace_tools_vendor ON public.vendor_marketplace_tools(vendor_id);
CREATE INDEX IF NOT EXISTS idx_voice_search_analytics_date ON public.voice_search_analytics(created_at);
CREATE INDEX IF NOT EXISTS idx_ar_vr_interactions_product ON public.ar_vr_interactions(product_id);
CREATE INDEX IF NOT EXISTS idx_mobile_app_analytics_platform ON public.mobile_app_analytics(platform);
CREATE INDEX IF NOT EXISTS idx_ml_model_performance_tracking_name ON public.ml_model_performance_tracking(model_name);
