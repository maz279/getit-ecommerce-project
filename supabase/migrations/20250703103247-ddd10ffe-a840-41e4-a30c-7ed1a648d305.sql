-- FINAL DATABASE AUDIT COMPLETION - Correct Column Names

-- 1. Create vendors (if not exists)
...
-- 5. Order items with correct columns
...
SELECT 
    ...
    jsonb_build_object('name', p.name, 'price', p.price, 'category', p.category_id)
FROM orders o
CROSS JOIN LATERAL (
...
VALUES 
    ('Weekend Flash Sale', 'Amazing discounts on selected products', NOW() + INTERVAL '1 day', NOW() + INTERVAL '3 days', 25, 1000, true, 'https://images.unsplash.com/photo-1607083206869-4c7672e72a8a?w=800', '["Limited time offer"]'::jsonb),
