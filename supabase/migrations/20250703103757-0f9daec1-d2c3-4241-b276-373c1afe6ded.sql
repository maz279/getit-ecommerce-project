-- FINAL DATABASE AUDIT SUCCESS - All Critical Business Tables

-- SUCCESS: Complete all critical business data with correct enums and constraints
...
-- 6. Vendor commissions (using correct commission_type enum values)
...
SELECT 
    ...
    'product_sale',  -- Correct enum value
    12.5,
    oi.total_price,
    ...
    CASE 
        WHEN o.status = 'delivered' THEN 'approved'  -- Correct enum value
        WHEN o.status IN ('shipped', 'processing') THEN 'calculated'  -- Correct enum value
        ELSE 'pending'  -- Correct enum value
    END,
    'unpaid',  -- Correct enum value
    o.created_at,
    'product_sales'
FROM order_items oi
...
SELECT 
    ...
    'Excellent product',
    'Great quality and fast delivery. Highly recommend this seller!',
    true,
...
SELECT 
    u.id,
    CASE 
        WHEN RANDOM() < 0.2 THEN 'laptop computer'
        WHEN RANDOM() < 0.4 THEN 'women dress'
        WHEN RANDOM() < 0.6 THEN 'mobile phone'
        WHEN RANDOM() < 0.8 THEN 'electronics'
        ELSE 'fashion'
    END,
    'text',
    ...
    NOW() - (RANDOM() * INTERVAL '30 days')
FROM users u
CROSS JOIN generate_series(1, 75);
...
INSERT INTO recently_viewed (user_id, product_id, vendor_id, viewed_at, view_duration_seconds, referrer_source)
SELECT 
    u.id,
    p.id,
    p.vendor_id,
