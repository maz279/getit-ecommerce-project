-- Product Service - Enhanced Database Schema for 100% Microservice Architecture
...
CREATE TABLE public.dynamic_pricing_rules (
    ...
    rule_type TEXT NOT NULL, -- 'discount', 'surge', 'competitor_based', 'demand_based'
    base_price NUMERIC(10,2) NOT NULL,
    min_price NUMERIC(10,2),
    max_price NUMERIC(10,2),
    discount_percentage NUMERIC(5,2),
    conditions JSONB NOT NULL, -- Triggering conditions
    time_based_rules JSONB, -- Festival pricing, flash sales
    inventory_based_rules JSONB, -- Low stock surge pricing
    competitor_based_rules JSONB, -- Price matching rules
    is_active BOOLEAN DEFAULT true,
...
CREATE POLICY "Vendors can manage their pricing rules" ON public.dynamic_pricing_rules
    FOR ALL USING (vendor_id = auth.uid() OR is_admin_user());
...
CREATE INDEX idx_dynamic_pricing_product_id ON public.dynamic_pricing_rules(product_id);
CREATE INDEX idx_dynamic_pricing_active ON public.dynamic_pricing_rules(is_active, priority) WHERE is_active = true;
...
CREATE TRIGGER update_product_categories_enhanced_updated_at BEFORE UPDATE ON public.product_categories_enhanced FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_product_attributes_bd_updated_at BEFORE UPDATE ON public.product_attributes_bd FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_inventory_automation_rules_updated_at BEFORE UPDATE ON public.inventory_automation_rules FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_product_media_enhanced_updated_at BEFORE UPDATE ON public.product_media_enhanced FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_recommendation_models_enhanced_updated_at BEFORE UPDATE ON public.recommendation_models_enhanced FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();